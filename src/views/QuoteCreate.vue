...
onMounted(async () => {
  ...
  // Best-effort - a quote doesn't have to draw from inventory, so this
  // failing shouldn't block the form working with free-text items.
  try {
...
// Same auto-fill behaviour as InvoiceCreate.vue's item picker - unlike
// invoice creation, picking an inventory item here does NOT reserve/deduct
// stock (see quote.service.js) - it's purely informational until the quote
// is actually converted to an invoice.
...
async function onSubmit() {
  ...
  try {
    ...
    router.push({ name: 'quote-detail', params: { code: res.data.quoteNumber } })
  } catch (e) {
    ...
    <form class="mt-6 space-y-6" @submit.prevent="onSubmit">
      ...
      <section class="card p-5">
            ...
            <div class="flex items-center gap-2">
              <select v-model="item.inventoryItemId" class="input flex-1" @change="onInventorySelect(item)">
                ...
                <option v-for="inv in inventory" :key="inv._id" :value="inv._id">
                  {{ inv.name }} — {{ formatMoney(inv.unitPrice, 'NGN') }}
                </option>
              </select>
              <button
                type="button"
        ...
        <p class="mt-3 text-xs text-ink-400">
          Picking an inventory item here doesn't reserve stock - only converting this quote to an invoice does.
        </p>
      ...
      <div class="flex justify-end gap-3">
        <router-link :to="{ name: 'quotes' }" class="btn-secondary">Cancel</router-link>
        <button type="submit" class="btn-primary" :disabled="loading">{{ loading ? 'Creating…' : 'Create quote' }}</button>
      </div>
    </form>
  </div>
</template>
